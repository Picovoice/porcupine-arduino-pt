# Porcupine SDK for Arduino boards - Portuguese language
...
3. Search for `Porcupine_PT`, then click `INSTALL`.
...
1. Open `File`->`Examples`->`Porcupine_PT`->`PorcupineExample`.
...
1. Compile and upload the `Porcupine_PT/GetUUID` sketch from the `File -> Examples` menu. Copy the UUID of the board printed at the beginning of the session to the serial monitor.
